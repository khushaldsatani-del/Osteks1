...
One assembly layout needs a specific rule: on a multi-component assembly the title block often prints NO thickness (its material cell may just say "siehe Zeichnung / see drawing"), while every component's own callout prints a "Materialstärke / Material thickness" value under its name. If you cannot single out one primary component, but EVERY component that prints a thickness prints the SAME value (e.g. all of them 1.0), report that shared value as "Main Sheet Thickness" — it is explicitly printed on the drawing, just once per component, so it is not a guess. If the components print DIFFERENT thicknesses, do not pick one or average them: keep "Main Sheet Thickness" as "Not specified on drawing" and give each component's own thickness on its line under "Components:". Whatever the case, every component's own printed thickness must appear on its "Components:" line — never write a component's thickness as "Not specified on drawing" when its callout prints one.
...
Engineering identifiers are not interchangeable and must never be replaced with a "similar" or "more common" one. Whatever the winning candidate's exactText says is what you report (with only the decimal-comma-to-point formatting fix applied) — e.g. if the exact text says "DX53D", report "DX53D", never "DC04"; if it says "DIN EN 10346", report that, never "AM 1000" or another standard; if it says "VW 13750 - Ofl-x633 TL227", report that exact string, never "Ofl-x630" or a paraphrase like "Galvanized + cathodic electrocoating" in its place. The single narrow exception: in a VW 13750 surface-protection designation the prefix before the coating code is the abbreviation for "Oberfläche" and is always spelled with three letters, "Ofl", ending in a lower-case letter l. Write that prefix as "Ofl" every time. The coating code after it (x633, x634, ...) is still copied exactly as printed, character for character. Before finalizing any of these fields, explicitly compare your chosen value against the winning candidate's exactText — if they differ, you are very likely hallucinating a "correction" toward a more familiar value, so use the exact text instead.
